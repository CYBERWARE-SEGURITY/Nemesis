    ...
    public class MouseGdis
    {
        ...
        public static void MovingIconSnakeEffect()
        {
            ...
            try
            {
                ...
                int xOffset = 0;
                double amplitude = 22; // Amplitude da onda
                double frequency = 32; // Frequência da onda
                int yPos = h / 2; // Posição vertical inicial

                while (true)
                {
                    BitBlt(hdc, 0, 0, w, h, IntPtr.Zero, 0, 0, TernaryRasterOperations.SRCCOPY);
